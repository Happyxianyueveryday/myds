...
{
    public: 
    static void bubble_sort(T data[], int size);    // 冒泡排序：稳定 
    static void insert_sort(T data[], int size);    // 插入排序：稳定 
    static void select_sort(T data[], int size);    // 选择排序：数据不敏感 
    static void merge_sort(T data[], int size);     // 归并排序：稳定 
    static void quick_sort(T data[], int size);     // 快速排序 
    static void heap_sort(T data[], int size);      // 堆排序：数据不敏感 
    static void shell_sort(T data[],int size);      // 希尔排序 
    
    private:
    static void real_merge(T data[], int begin, int end);
    static void real_quick(T data[], int begin, int end);
    static void build_heap(T data[], int size);
};
 ...
 */
template<typename T> 
void Sort<T>::bubble_sort(T data[], int size) 
{
 ...
 */
template<typename T> 
void Sort<T>::insert_sort(T data[], int size)
{
 ...
 */
template<typename T> 
void Sort<T>::select_sort(T data[], int size)
{
	for(int i=0;i<size;i++)    // 要交换的元素的下标 
	{
		int index=i;
		T min=data[i];   // 后(size-i)个元素中的最小值和所在下标 
		...
		T temp=data[i];        // 交换后(size-i)个元素中的最小值元素和位置i的元素 
		data[i]=min;
		data[index]=temp;
	}
}
 ...
 */
template<typename T> 
void Sort<T>::merge_sort(T data[], int size)
{
	Sort::real_merge(data, 0, size-1);
}

template<typename T> 
void Sort<T>::real_merge(T data[], int begin, int end)
{
	if(begin>=end)
	return;
	else
 ...
 */
template<typename T> 
void Sort<T>::quick_sort(T data[], int size)
{
	real_quick(data, 0, size-1);
}

template<typename T> 
void Sort<T>::real_quick(T data[], int begin, int end)
{
	if(begin>=end)
	return;
	
	int pos=begin;           // 最后一个小于主元的元素组成的序列位置的下标 
	T pivot=data[begin];     // 主元的值 
	...
	T temp=data[begin];     // 交换主元和最后位置下标的元素 
	data[begin]=data[pos];
	data[pos]=temp;
 ...
 */
template<typename T> 
void Sort<T>::heap_sort(T data[], int size)
{
	Sort<T>::build_heap(data, size);
	
	for(int i=size-1;i>0;i--)   // 依次将大根堆堆顶元素和最大值位置，次大值位置，...，进行交换 
	{
		T temp=data[0];   // 交换data[i]和data[0] 
		data[0]=data[i];
		data[i]=temp;
		...
		{
			T left=(pos*2+1<i)?data[pos*2+1]:T(INT_MIN);
			T right=(pos*2+2<i)?data[pos*2+2]:T(INT_MIN);
 ...
 */
template<typename T> 
void Sort<T>::build_heap(T data[], int size)
{
	for(int i=size/2-1;i>=0;i--)
	{
		T left=(2*i+1<size)?data[2*i+1]:T(INT_MIN);   // 左子结点的值，注意判断左右子结点是否确实存在 
		T right=(2*i+2<size)?data[2*i+2]:T(INT_MIN);  // 右子结点的值
 ...
 */
template<typename T> 
void Sort<T>::shell_sort(T data[], int size)
{
	int step=size/2;
	while(step>=1)
	{
