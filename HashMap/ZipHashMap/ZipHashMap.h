...
class ZipHashMap: public HashMap<T, R> 
{
    ...
    ZipHashMap(const vector<T> &_keys, const vector<R> &_values); // 从数组构造哈希表
    ...
    vector<T> keys() const override;              // 获得哈希表中所有值的列表
 ...
 */
template<typename T, typename R>
ZipHashMap<T, R>::ZipHashMap(const vector<T> &_keys, const vector<R> &_values)
{
	size=0;    									// 初始化元素个数为0
	capacity=16;        						// 默认桶的数量初始值为16，16为2的幂，并且每次扩容均乘以2保持桶数量均为2的整数次幂 
	load_factor=0.75;   						// 默认的负载因子初始值为0.75
 ...
 */
template<typename T, typename R>
vector<T> ZipHashMap<T, R>::keys() const
{
	vector<T> res;
	if(!size)
	return res;
 ...
 */
template<typename T, typename R>
string ZipHashMap<T, R>::toString() const
{
    string res;
    for(int i=0;i<capacity;i++)
    {
