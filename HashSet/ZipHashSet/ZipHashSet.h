...
/**
 * ZipHashSet: 移动构造函数 
 * note: 移动构造函数使用浅拷贝，借助cpp11的移动语义提供安全的浅拷贝/移动手段 
 */
...
bool ZipHashSet::contain(const HashElement *value) const 
{
	...
	while(now)
	{
		if(now->val->hashCode()==value->hashCode()&&now->val->equals(value))  // 先比较hashCode()方法的返回值是否相等，若相等再比较equals()方法来进行进一步比较 
		return true;
		else
		now=now->next;   
	}
