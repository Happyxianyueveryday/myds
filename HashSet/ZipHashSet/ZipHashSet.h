...
void ZipHashSet<T>::resize() 
{
	...
	for(int i=0;i<old_capacity;i++)
	{
		...
		while(now)
		{
			unsigned int index=hash(now->val);        // 计算新的哈希地址和位置 
 ...
 */ 
template<typename T>
unsigned int ZipHashSet<T>::hash(const T &value) const
{
	unsigned int hashCode=value.hashCode();
	unsigned int finalhash=(hashCode>>16)^hashCode;
	return finalhash&(capacity-1);
}
