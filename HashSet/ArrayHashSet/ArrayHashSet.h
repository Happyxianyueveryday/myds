...
template<typename T>
class ArrayHashSet: public HashSet<T>
{
	public:
	// 1. constructor
	ArrayHashSet(string mode, unsigned int (*_rehash)(T value)=nullptr);  // 构造函数 
	
	// 2. copy/move controller
	ArrayHashSet(const ArrayHashSet<T> &other);   			   // 拷贝构造函数
	ArrayHashSet &operator= (const ArrayHashSet<T> &other);   // 拷贝赋值运算符 
	ArrayHashSet(ArrayHashSet<T> &&other);                    // 移动构造函数
	ArrayHashSet &operator= (ArrayHashSet<T> &&other);        // 移动赋值运算符 
	...
	private:
	// 5. domain
	const T **data;// 哈希元素存储数组 
	bool *sign;              // 哈希元素删除标志数组
	string mode;             // 选用的模式  
	int size;                // 哈希表中的元素数量 
	int max_size;            // 哈希表的桶数量
	unsigned int (*rehash)(T value);    // 再哈希法使用的再哈希法函数指针 
 ...
 */
template<typename T>
ArrayHashSet<T>::ArrayHashSet(string _mode, unsigned int (*_rehash)(T value))
{
	size=0;
	max_size=13;    // 默认的初始桶数量为13 
	data=new const T* [max_size];
 ...
 */
template<typename T>
ArrayHashSet<T>::ArrayHashSet(const ArrayHashSet &other)
{
	// 1. 深拷贝源对象成员 
	size=other.size;
 ...
 */
template<typename T>
ArrayHashSet<T> &ArrayHashSet<T>::operator= (const ArrayHashSet<T> &other)
{
	// 1. 判断是否自身赋值 
	if(&other==this)
	return (*this);
 ...
 */ 
template<typename T>
ArrayHashSet<T>::ArrayHashSet(ArrayHashSet<T> &&other)
{
	// 1. 浅拷贝源对象的成员 
	size=other.size;
	max_size=other.max_size;
	...
	// 2. 重置源对象成员为默认值
	other.size=0;
	other.data=new const T* [other.max_size];
 ...
 */
template<typename T>
ArrayHashSet<T> &ArrayHashSet<T>::operator= (ArrayHashSet<T> &&other)
{
	// 1. 处理自身赋值 
	if(&other==this)
	return (*this);
 ...
 */
template<typename T>
void ArrayHashSet<T>::insert(T value)
{
 ...
 */
template<typename T>
void ArrayHashSet<T>::remove(T value) 
{
	// 1. 当前哈希表判空 
	if(!size)
	return;
 ...
 */
template<typename T>
bool ArrayHashSet<T>::contain(T value) const 
{
	// 1. 当前哈希表判空 
	if(!size)
	return false;	
 ...
 */
template<typename T>
vector<T> ArrayHashSet<T>::values() const
{
	vector<T> res;
	for(int i=0;i<max_size;i++)
	{
 ...
 */
template<typename T>
int ArrayHashSet<T>::length() const
{
	return size;
}
 ...
 */
template<typename T>
string ArrayHashSet<T>::toString() const
{
	string res;
	for(int i=0;i<max_size;i++) 
	{
